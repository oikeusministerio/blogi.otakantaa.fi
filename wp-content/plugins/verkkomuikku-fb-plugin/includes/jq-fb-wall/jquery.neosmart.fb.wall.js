...
/**
 * Verkkomuikku edit:
 * use jStorage to cache the wall. Every 10 request updates the cache.
 */
(function($) {
	...
	$.fn.fbWall = function(options) {
		...
		return meta.each(function() {
			...
			var initBase = function(data){
				...
				// Verkkomuikku. Check if the data is in cache already
				var cached_data = $.jStorage.get('fb-wall');
				var cached_data_request = $.jStorage.get('fb-wall-request');
				
				// Keep track how many times the cache is read.
				// Every cache read reduces the number.
				if (!cached_data_request || cached_data_request == 0) {
					$.jStorage.deleteKey('fb-wall');
					$.jStorage.set('fb-wall-request', 10);
					//console.log("Data from cache counter set to 10. Cache flushed.");
				}
				
				if (cached_data && cached_data_request >= 1) {
					$.jStorage.set('fb-wall-request',(cached_data_request-1));
					meta.removeClass('loading');
					//console.log("Data from cache. Requests left #"+cached_data_request);
					initWall(cached_data);
				} else {
					$.ajax({
