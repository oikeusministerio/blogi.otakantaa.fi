...
function alo_em_show_widget_form ( ) {
	...
	// Verkkomuikku
	$dd = is_page(get_option('alo_em_subsc_page')) ? "<dd>&nbsp;</dd>" : "";
    ...
    if ( $mailinglists ) {
	    ...
	    // Verkkomuikku, use dl instead of table for nicer output
	    //$lists_table .= "<div class='alo_easymail_lists_table'>" . $lists_msg .":<br />";
	    //$lists_table .= "<table><tbody>\n";
	    $lists_table .= '<dl>'.$dd.'<dt>' . $lists_msg . ':</dt>';
		...
		foreach ( $mailinglists as $list => $val ) {
			...
			$lists_table .= $dd."<dt><input type='checkbox' name='alo_em_form_lists[]' id='alo_em_form_list_$list' value='$list' $checked $checkbox_js class='input-checkbox' />" . alo_em_translate_multilangs_array ( alo_em_get_language(), $val['name'], true ) . "</dt>\n";
	...
	if (is_user_logged_in()) {
        ...
        */
        ob_start();
?>
<div class="vmfeu_wrapper">
<dl>
	<?php echo $dd ?><dt><input onchange='alo_em_user_form("yes");return false;' type='radio' <?php echo $optin_checked; ?> name='alo_easymail_option' value='yes' class='input-radio' /><?php echo $optin_msg; ?></dt>
	<?php echo $dd ?><dt><input onchange='alo_em_user_form("no");return false;' type='radio' <?php echo $optout_checked; ?> name='alo_easymail_option' value='no' class='input-radio' /><?php echo $optout_msg; ?></dt>
</dl>
<?php 
		$html .= ob_get_contents();
        ob_end_clean();
        // end verkkomuikku edit
		...
		if( $alo_em_cf ):
			...
			foreach( $alo_em_cf as $key => $value ){
				// Verkkomuikku - dd / dt instead of table
				//$html .= "  <tr>\n";
				$field_id = "alo_em_".$key; // edit-by-alo
				// Verkkomuikku
				//$html .= "    <th><label for='".$field_id."'>". $value['humans_name'] ."</label></th>\n";
				//$html .= "    <td>\n";
				$html .= "<dd><label for='".$field_id."'>". $value['humans_name'] ."</label></dd><dt>";
        ...
        // Verkkomuikku end the vmfeu_wrapper div
		$html .= "</div>";        
        $html .= "</form>\n";
...
<div class="vmfeu_wrapper">
<dl>
	<dd><?php _e("Name", "alo-easymail"); ?></dd>
	<dt><input type='text' name='alo_em_opt_name' value='<?php echo $alo_em_opt_name ?>' id='opt_name' maxlength='50' class='input-text' /></dt>
	<dd><?php _e("E-mail", "alo-easymail"); ?></dd>
	<dt><input type='text' name='alo_em_opt_email' value='<?php echo $alo_em_opt_email ?>' id='opt_email' maxlength='50' class='input-text' /></dt>
</dl>
<?php 
		$html .= ob_get_contents();
	    ob_end_clean();
		...
		if( $alo_em_cf ):
			foreach( $alo_em_cf as $key => $value ){
				...
				//$html .= "    <td>\n";
				$html .= "<dd><label for='".$field_id."'>". __( $value['humans_name'], "alo-easymail") ."</label></dd><dt>";				
        ...
        $html .= "<dl>";
		$html .= $dd . "<dt><input type='submit' name='submit' value='".__("Subscribe", "alo-easymail")."' class='input-submit' /></dt>";
		$html .= "</dl>";
		$html .= "</div>"; // vmfeu_wrapper        
        $html .= "</form>\n";    
    } 
