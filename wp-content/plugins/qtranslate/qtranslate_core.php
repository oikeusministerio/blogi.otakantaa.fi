...
function qtrans_init() {
	...
	// verkkomuikku - this function needs to be called after init is fully done.
	// Current user can... -part moved into qtrans_updateTermLibrary since it cannot be called here
	...
	// Verkkomuikku - disable detect_browser_language, fails when qtranslate is modded
	
	if(0) { // $q_config['detect_browser_language'] && $q_config['url_info']['redirect'] && !isset($_COOKIE['qtrans_cookie_test']) && $q_config['url_info']['language'] == $q_config['default_language']) {
		$target = false;
...
function qtrans_updateTermLibrary() {
	...
	// Verkkomuikku || !current_user_can('manage_categories') added
...
function qtrans_use($lang, $text, $show_available=false) {
	...
	// Verkkomuikku
	// Content that is not available should be hidden..
	return "";
